...
# IPv4 rules
iptables -I INPUT -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
iptables -I FORWARD -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
iptables -I FORWARD -o "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
iptables -t nat -I POSTROUTING -s "$SUBNET" -o "$DEFAULT_IF" -j MASQUERADE 2>> /tmp/nm-dispatcher.log
iptables -I INPUT -i "$DEFAULT_IF" -p udp --dport "${WG_PORT}" -j ACCEPT 2>> /tmp/nm-dispatcher.log

# IPv6 rules
ip6tables -I INPUT -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
ip6tables -I FORWARD -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
ip6tables -I FORWARD -o "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
ip6tables -I INPUT -i "$DEFAULT_IF" -p udp --dport "${WG_PORT}" -j ACCEPT 2>> /tmp/nm-dispatcher.log
