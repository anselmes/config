...
# Remove IPv4 rules
iptables -D INPUT -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
iptables -D FORWARD -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
iptables -D FORWARD -o "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
iptables -t nat -D POSTROUTING -s "$SUBNET" -o "$DEFAULT_IF" -j MASQUERADE 2>> /tmp/nm-dispatcher.log
iptables -D INPUT -i "$DEFAULT_IF" -p udp --dport "${WG_PORT}" -j ACCEPT 2>> /tmp/nm-dispatcher.log

# Remove IPv6 rules
ip6tables -D INPUT -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
ip6tables -D FORWARD -i "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
ip6tables -D FORWARD -o "$IFACE" -j ACCEPT 2>> /tmp/nm-dispatcher.log
ip6tables -D INPUT -i "$DEFAULT_IF" -p udp --dport "${WG_PORT}" -j ACCEPT 2>> /tmp/nm-dispatcher.log
